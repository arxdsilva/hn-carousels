...
3. Blog post. Only after the carousel was sent to Metricool successfully:
   ...
   c. Write a new post in that format, 800–1200 words, on the same topic as today's carousel. Go deeper than the carousel: my practical take, the strongest counterpoints from the discussion, and what a developer should do differently. Credit and link the original article. Never copy passages from the source.
   d. Use the create-content skill for the title, meta description, slug and heading structure, without keyword stuffing. Use schema-markup only if the site doesn't already generate article schema. Then run brand-voice-enforcement (if .claude/brand-voice-guidelines.md exists) and humanizer on the text.
   e. Nobody reviews the blog before it goes live, so: first-person opinions are fine ("I think", "my advice"), but never invent personal experiences or anecdotes about me or my team. Every fact, number and quote must come from the article or the discussion. If the article couldn't be fetched, skip the blog for this run.
   f. Install dependencies and run the site's build. If the build fails, don't push; include the error in the owner email.
   g. Commit only the new post as "blog: <slug>" and push to main of ytwebsite.
